...
Uses Azure Container Registry (ACR) remote build – no local Docker required.
...
main() {
    ...
    # ── Build container image (ACR remote build) ────────────────────────
    header "Building container image"

    if [[ "${DRY_RUN}" == "true" ]]; then
        info "[DRY RUN] Would build image remotely in ACR from ${REPO_ROOT}/mcp_servers"
    ...
    # Build image remotely in ACR (no local Docker/Podman required)
    local full_image="${acr_server}/ess-mcp:${IMAGE_TAG}"
    info "Building image remotely in ACR: ${full_image}"
    az acr build \
        --registry "${acr_name}" \
