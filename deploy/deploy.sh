...
main() {
    ...
    header "Building container image (ACR remote build)"
    ...
    local full_image="ess-mcp:${IMAGE_TAG}"
    info "Building image remotely in ACR: ${acr_server}/${full_image}"
    az acr build \
        --registry "${acr_name}" \
        --image "${full_image}" \
        --file "${REPO_ROOT}/mcp_servers/Dockerfile" \
        "${REPO_ROOT}/mcp_servers"
    ok "Image built and pushed: ${acr_server}/${full_image}"
